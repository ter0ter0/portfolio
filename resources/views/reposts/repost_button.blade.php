...
@if (Auth::check())
    ...
    @endif
@else
    <a href="{{ route('login') }}" class="btn m-0 p-1 shadow-none" data-toggle="tooltip" title="ログインしてリポスト"><i class="bi bi-arrow-repeat"></i></a>
@endif
<span id="repost-count-{{ $post->id }}" class="badge badge-pill">{{ $post->repostedPosts()->count() }}</span>
